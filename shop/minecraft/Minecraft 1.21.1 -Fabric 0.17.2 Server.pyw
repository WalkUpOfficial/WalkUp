...
OPENJDK_URL = 'https://aka.ms/download-jdk/microsoft-jdk-21-windows-x64.msi'

System32.computer.Get_Administrtor_permissions()
...
class main:
    ...
    def __init__(self):
        ...
        opu(text='')
        tm.sleep(0.5)
        self.path = filedialog.askdirectory(title='选择安装服务器的位置')
        tm.sleep(0.5)
        ...
        m = re.search(r'version "(\d+)', r.stderr)
        if m and m.group(1) == '21':
            opu(text='你似乎没有安装Java环境')
            tm.sleep(2)
            opu(text='如果没有Java环境')
            tm.sleep(1.5)
            opu(text='！服务器就无法运行 ！')
            tm.sleep(2)
            opu(text='需要下载Java环境吗？')
            tm.sleep(1.5)
            if messagebox.askyesno('', ''):
                urllib.request.urlretrieve(OPENJDK_URL, os.path.join(os.environ['USERPROFILE'], 'AppData', 'Local', 'Temp')+r'\OpenJDK Setup.msi')
                subprocess.run(["msiexec", "/i", os.path.join(os.environ['USERPROFILE'], 'AppData', 'Local', 'Temp')+r'\OpenJDK Setup.msi'], shell=True)
                opu(text='Java 环境已安装完毕')
        opu(text='正在部署服务器 ...')
        main.install(self.path)
        opu(text='安装完成')
